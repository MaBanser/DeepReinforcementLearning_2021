...
class SampleManager:
    ...
    def __init__(
        ...
        if not ("input_shape" in kwargs):
            ...
            self.kwargs["input_shape"] = state.shape
        ...
        if not('weights' in kwargs.keys()):
            ...
            self.kwargs['weights'] = random_weights
    ...
    def initialize_weights(self, model, input_shape, model_kwargs):
        ...
        if not(input_shape):
            print('You can’t be here, it is impossible')
    ...
    def _store(self, results):
        ...
        ), "data keys and return keys do not match"
